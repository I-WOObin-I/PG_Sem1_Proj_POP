...
void task_8()
{
    ...
    card** players_cards = new card *[ game_settings.players_C * 2 ];         //// setting all players cards array          //
    for(int i=0 ; i<game_settings.players_C * 2 ; i++)                          // it has both "in hand" & "on deck"        //
    {                                                                           // cards                                    //
        players_cards[ i ] = new card [ 10000 ];                                //
    }                                                                         ////
    ...
    for(int i=0 ; i<game_settings.players_C * 2 ; i++)                                    //// reading input for players hands & decks  //
    {                                                                                       //
        cin.getline( input_line , line_L );                                                 //
        players_cards_C[i] = interpret_input( input_line , players_cards[i] , 20 );         // does both, interprets input and returns  //
    }                                                                                     //// how many cards were found

    for(int i=0 ; i<game_settings.players_C * 2 ; i++)                                    //// counts how many cards in each            //
    {                                                                                       // color exist                              //
        count_colors_number( spec_color_C , players_cards[i] , players_cards_C[i] );        //
    }                                                                                     ////

    game_settings.color_C = count_colors_by_num( spec_color_C );                            // counts non green colors aka. cauldrons   //
    ...
    for(int i=0 ; i<game_settings.color_C ; i++)                                          //// reading input for cauldrons              //
    {                                                                                       //
        cin.getline( input_line , line_L );                                                 //
        cauldrons_cards_C[i] = interpret_input( input_line , cauldrons_cards[i] , 13 );     // 13 is address of first card in input     //
    }                                                                                     ////
